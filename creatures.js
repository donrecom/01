...
// ! 2.  BEGIN of the Program
  ...
  var LogSwitch={} // tags for logging
...
/** { Log parts switches. Put something in quotation marks to include the corresponding log.
  ...
  */
      LogSwitch = {
        ...
        statisticForYear: statisticForYear,
        StatEvery5genOrZeroing: StatEvery5genOrZeroing,
        ParametersNewCreature: ParametersNewCreature,
        Dialogs: Dialogs,
        typeWhoBorn: typeWhoBorn,
        detaleWhoBorn: detaleWhoBorn,
        typeWhoDiedAge: typeWhoDiedAge,
      };
      InstalCreaturesData = {
...
// ! 3.1 пишем лог
function Log(txt,clName){
console.log(txt);
if (pause_==1){
  let childP;
let lenP=document.getElementsByTagName("p").length
while (lenP>1000){
  childP=document.getElementsByTagName("p")[0]
  childP.parentNode.removeChild(childP)
  lenP--
  }
...
function newYear(InstalCreaturesData) {
  ...
  if (LogSwitch.statisticForYear != false) statistic();
...
function BornType(countAll, countFem, CreaturesType, subType) {
  ...
  for (CreatureOfType = 1; CreatureOfType <= countAll; CreatureOfType++) {
    if (allCreatures.length <= 5000000) { 
      ...
      if (LogSwitch.ParametersNewCreature != false)
...
function meetingtolk(Creature = Array(2)) {
  if (LogSwitch.Dialogs != false) historyWrite([Creature[0], Creature[1]]); // Сompose meeting history // todo> --> Look   historyWrite (Creature=Array(2))
  ...
  if (LogSwitch.Dialogs != false) DialogWrite([Creature[0], Creature[1]]); // Сompose meeting dialog // todo> --> Look   DialogWrite (Creature=Array(2))
}
...
function historyWrite(Creatures = Array(2)) {
  ...
  Log(history,"meeting"); // write meeting history
}
...
function DialogWrite(Creature = Array(2)) {
 ...
 ${NameSubType2}: -Приветствую ${hi1} ${NameSubType1}! ${NewSmile2}`; // -Hello
  Log(dialog,"dialog"); // write meeting dialog
}
...
function DeathСheck(i) {
  if (allCreatures[i].lifespan - allCreatures[i].age <= 0) {
    ...
    if (LogSwitch.typeWhoDiedAge != false)
       {
        let fem0
        if (allCreatures[i].gender == "женщина") {fem0= 'а'} else {fem0=''};
    Log(`${allCreatures[i].subType} ${allCreatures[i].name}, чей возраст был ${allCreatures[i].age}${years(allCreatures[i].age)} познал${fem0} смерть`,"death");
  }
...
//! 24  flags switch for monitoring statistics
function chbox() {
  let v;
